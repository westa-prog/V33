...
export const DriverTable: React.FC<DriverTableProps> = ({
  ...
  const companyNames = useMemo(() => {
    const fromCatalog = companies.map(c => c.name).filter(Boolean);
    const list = Array.from(new Set([...fromCatalog, ...drivers.map(d => d.company).filter(Boolean)]));
    return list.sort();
  }, [companies, drivers]);
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      {(driver.createdByName || driver.createdByEmail) && (
                        <div className="text-[10px] font-semibold text-indigo-600 dark:text-indigo-400 mt-0.5">
                          Added by: {driver.createdByName || driver.createdByEmail}
                        </div>
                      )}
                    </div>
                  </div>
